...
echo "Installing yay..."
sudo pacman -S --no-confirm --needed base-devel && git clone https://aur.archlinux.org/yay-bin.git && cd yay-bin && makepkg -si

echo "Installing required packages..."
sudo pacman -S --no-confirm meson gcc cpio sddm qt6-svg qt6-virtualkeyboard qt6-multimedia-ffmpeg pamixer fzf hyprpolkitagent awww mako rofi nautilus waybar wl-clipboard stow firefox ttf-cascadia-mono-nerd neovim matugen fastfetch zsh hypridle eza zoxide bat impala bluetui hyprlock wget ghostty

yay -S --no-confirm rmtrash sddm-silent-theme python-pywalfox hyprshot-git
...
read -p "Do you want to install marware's favorite apps? (Y/n): " choice

case "$choice" in
y | Y | yes | Yes | YES)
  sudo pacman -S helium-browser-bin steam vesktop-bin prismlauncher libreoffice-fresh qbittorrent gimp localsend inkscape audacity gnome-disk-utility vlc
  yay -S lunar-client
  ;;
n | N | no | No | NO) ;;
*) ;;
